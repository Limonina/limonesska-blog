...
## Личное пространство Лионы (`My way/`)

`My way/` — отдельный **приватный** репозиторий внутри сайта (свой `.git`, ветка `master`, в `.gitignore` сайта, **никогда не пушится**): личные планы, идеи постов, финансы, материалы и паспорта (`My way/Китай/`).

**В начале работы прочитай `My way/CLAUDE.md`** — там личный контекст: кто такая Лиона и как с ней общаться, как подходить к работе, чего НЕ предлагать, структура папки, визуальный язык сайта, скилл `/идея`. Этот файл **приватный** — его содержимое (особенно финансы, психопрофиль, паспорта) **не копировать** в отслеживаемые файлы сайта: они уходят на публичный GitHub. Личное остаётся только в `My way/` (gitignore).

Ключевые рабочие файлы: `My way/Планы/Сводка идей.md` (таблица идей постов), `My way/Планы/структура-сайта.md` (архитектура блога), `My way/Идеи постов/` (отдельные идеи). Связывай их с кодом сайта — всё в одном агенте, `/add-dir` не нужен.
