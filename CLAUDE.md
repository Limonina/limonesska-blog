...
Локальный лог работы в формате *дата · время · Запрос · Recap* (в `.gitignore`, UTF-8, правится напрямую). Запрос и уточняющие вопросы пишут хуки автоматически (`.claude/hooks/log-prompt.ps1` на `UserPromptSubmit`, `log-question.ps1` на `PostToolUse`/`AskUserQuestion`; подключены в `settings.local.json`). **Их вручную не дублировать.**

**Recap пишет Claude.** Сразу после выполнения задачи допиши в её запись журнала: короткий заголовок после `### ЧЧ:ММ — ` и строку `**Recap:** ...` под `**Запрос:**` (что сделано, ключевые файлы/решения, хеш коммита если коммитил). Если несколько коротких сообщений сложились в одну задачу — recap к последней. Не редактируй и не удаляй строки `**Запрос:**`.
