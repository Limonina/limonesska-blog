...
const entrySchema = z.object({
  ...
  // Куда смещать кадр главного фото в квадратной ячейке календаря (object-position).
  // Не задано — центр. Примеры: "70% 50%" (правее), "50% 20%" (выше).
  photoPos: z.string().optional(),
  // Метка достоверности для постов базы знаний: проверено лично или нет.
  // Не задано — метка не показывается.
  verified: z.boolean().optional(),
  // Анонс: запись видна в списках как тизер, но её страница показывает
  // заглушку «выйдет такого-то» (дата = pubDate) вместо текста. ≠ draft.
